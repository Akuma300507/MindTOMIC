...
export const excelService = {
  ...
  exportFullEventReport(db: AppDatabase) {
    ...
    const settingsRows = [
      ...
      { 'Setting': 'Round 1 (Pixel Pictionary) Speech Time (s)', 'Value': db?.settings?.round1?.speechTimeSeconds ?? 180 },
      { 'Setting': 'Round 2 (Arcade Wheel) Active Wheel Topics', 'Value': db?.settings?.round2?.activeWheelTopicCount ?? 10 },
      { 'Setting': 'Round 2 (Arcade Wheel) Prep Time (s)', 'Value': db?.settings?.round2?.prepTimeSeconds ?? 0 },
      { 'Setting': 'Round 2 (Arcade Wheel) Speech Time (s)', 'Value': db?.settings?.round2?.speechTimeSeconds ?? 480 },
      { 'Setting': 'Round 3 (The Mystery Cartridge) Speech Time (s)', 'Value': db?.settings?.round3?.speechTimeSeconds ?? 300 },
