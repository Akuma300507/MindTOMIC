...
export const SpinRevealCardModal: React.FC<SpinRevealCardModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen && topic) {
      setFlipped(false);
      // Flip after a brief dramatic delay
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 overflow-hidden">
      ...
      <div className="absolute w-[500px] h-[500px] rounded-full bg-gradient-to-tr from-amber-500/20 via-purple-600/30 to-cyan-500/20 blur-3xl pointer-events-none" />

      {/* Floating Global Close Button for Guaranteed Exit */}
      {onClose && (
        <button
          onClick={onClose}
          className="absolute top-4 right-4 sm:top-6 sm:right-6 z-50 p-2.5 rounded-full bg-slate-900/90 hover:bg-slate-800 text-slate-300 hover:text-white border border-slate-700 shadow-2xl transition-all hover:scale-105 active:scale-95 cursor-pointer"
      ...
      <div className="relative z-10 w-full max-w-lg sm:max-w-xl md:max-w-2xl flex items-center justify-center">
        <AnimatePresence mode="wait">
          {!flipped ? (
            <motion.div
              key="card-back"
              initial={{ scale: 0.2, opacity: 0, rotateY: 0 }}
              animate={{ scale: 1, opacity: 1, rotateY: 0 }}
              exit={{ scale: 0.85, opacity: 0, rotateY: 90 }}
              transition={{ duration: 0.35, ease: 'easeOut' }}
              className="w-full min-h-[380px] sm:min-h-[420px] md:min-h-[460px] rounded-3xl p-6 sm:p-8 bg-gradient-to-br from-slate-900 via-slate-950 to-indigo-950 border-[3px] border-amber-400/90 shadow-[0_0_60px_rgba(245,158,11,0.45)] flex flex-col items-center justify-center relative overflow-hidden"
            >
              ...
              <div className="my-auto flex flex-col items-center">
                ...
                <div className="mt-5 flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-amber-400/10 border border-amber-400/30 text-amber-300 font-mono text-[11px] font-bold tracking-widest uppercase shadow-md">
            ...
            <motion.div
              ...
              className="w-full min-h-[380px] sm:min-h-[420px] md:min-h-[460px] rounded-3xl p-6 sm:p-8 md:p-10 bg-gradient-to-br from-slate-900 via-slate-950 to-indigo-950 border-[3px] border-amber-400/95 shadow-[0_0_70px_rgba(245,158,11,0.5)] flex flex-col justify-between relative overflow-hidden"
            >
              {/* Metallic Corner Ornaments */}
              <div className="absolute top-3 left-3 w-6 h-6 border-t-2 border-l-2 border-amber-400/80 rounded-tl-lg" />
              <div className="absolute top-3 right-3 w-6 h-6 border-t-2 border-r-2 border-amber-400/80 rounded-tr-lg" />
                ...
                <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
                  {onReplaceOnWheel && (
                    <button
                      ...
                      className="flex-1 sm:flex-initial px-3 py-2 rounded-xl bg-purple-950/70 hover:bg-purple-900 text-purple-200 border border-purple-700/60 text-xs font-bold transition-all cursor-pointer"
                  ...
                  {onStartTimer ? (
                    <button
                      ...
                      className="flex-1 sm:flex-initial px-5 py-2 rounded-xl bg-gradient-to-r from-amber-500 via-amber-400 to-yellow-500 hover:from-amber-400 hover:to-yellow-400 text-slate-950 font-black text-xs sm:text-sm uppercase tracking-wider shadow-lg shadow-amber-500/30 transition-all active:scale-95 flex items-center justify-center gap-2 cursor-pointer"
                    >
                    ...
                    <button
                      onClick={onClose}
                      className="flex-1 sm:flex-initial px-5 py-2 rounded-xl bg-gradient-to-r from-amber-500 via-amber-400 to-yellow-500 hover:from-amber-400 hover:to-yellow-400 text-slate-950 font-black text-xs sm:text-sm uppercase tracking-wider shadow-lg shadow-amber-500/30 transition-all active:scale-95 cursor-pointer"
                    >
                      Ready for Stage
                    </button>
                  )}
